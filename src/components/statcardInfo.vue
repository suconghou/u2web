	...
	<div class="stat-card-info">
		...
		<mu-paper :z-depth="1">
			<div class="v-stat">
				...
				<statcard ref="v" :title="names.vtitle" :segments="vdispatch" :datas="vdatas" />
			</div>
		</mu-paper>
		<mu-paper :z-depth="1">
			<div class="v-stat">
				<div class="type-title">音频统计</div>
				<statcard ref="a" :title="names.atitle" :segments="adispatch" :datas="adatas" />
			</div>
		</mu-paper>
	</div>
</template>
		...
		onLoadersready(
			...
			if (vloader) {
				...
				vloader.listen("res.start", item => {
					// 某一文件块,要开始下载了
					t.v && t.v.statusupdate("start", item);
				});
				vloader.listen("res.done", res => {
					// 某一文件块已完成,可能成功,可能失败
					t.v && t.v.statusupdate("done", res);
				});
			...
			if (aloader) {
				...
				aloader.listen("res.start", item => {
					// 某一文件块,要开始下载了
					t.a && t.a.statusupdate("start", item);
				});
				aloader.listen("res.done", res => {
					// 某一文件块已完成,可能成功,可能失败
					t.a && t.a.statusupdate("done", res);
				});
